...
class TestGeocode(unittest.TestCase):
    def setUp(self):
        ...
        self.la_point = EarthPoint(self.la_long, self.la_lat)
        self.sf_point = EarthPoint(self.sf_long, self.sf_lat)

    def test_get_distance_should_calculate_correctly(self):
        distance = get_distance_meters(self.la_point, self.sf_point)
    ...
    def test_get_distance_should_return_zero_for_same_point(self):
        point = EarthPoint(1.23, 1.23)
        distance = get_distance_meters(point, point)
        self.assertAlmostEqual(distance, 0, places=2)

    def test_get_at_distance_from_should_move_north_correctly(self):
        start = EarthPoint(-122.45, 37.73)  # SF coordinates
        result = get_at_distance_from(start, 5000, "N")
    ...
    def test_get_at_distance_from_should_move_south_correctly(self):
        start = EarthPoint(-122.45, 37.73)  # SF coordinates
        result = get_at_distance_from(start, 5000, "S")
    ...
    def test_get_at_distance_from_should_move_east_correctly(self):
        start = EarthPoint(-122.45, 37.73)  # SF coordinates
        result = get_at_distance_from(start, 5000, "E")
