...
- **Follow-up — promote ForeverTree to a guide.** Add the ForeverTree paper example to the guides section as the canonical paper example, alongside the existing `hello` / `grass_shrub_fire` / `two_trees` guides — i.e. a `landing/guides/forevertree.html` (hand-authored, like the others) linked from `landing/guide.html`, a `docs/guide/md/forevertree.md`, and wiring its example/data into the guide build (mirror how the others copy `examples/guide/*.josh` and preprocess tutorial data in CI). The External-Data figures, the `.jshd` data, and the WASM playground built here can be reused/linked. Not done yet — separate from this demo PR.
