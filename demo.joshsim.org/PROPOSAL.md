...
_External-data figures (substep 4):_ the External Data substep shows four static images from `paper/forevertree/figures/` (temperature + precipitation spatial rasters and their growth-response curves) so the climate inputs and why the response relationships make sense are visible. Canonical source stays in `paper/forevertree/figures/`; `install_deps.sh` copies them into the gitignored `demo/img/` (same pattern as the data files). Chose static images (Option C) over a live probe-sim render for minimal lift.
...
Copied the editor's WASM stack verbatim into `demo/js/` (`wasm.js`, `wasm.worker.js`, `wire.js`, `model.js`, `debug.js`, `util.js`, `parse.js`) and added `demo/war/get_from_jar.sh` (extracts the TeaVM WASM from `build/libs/JoshSim.war` into the gitignored `demo/war/`). `PlaygroundPresenter` now runs the sim on Run: it fades in a "Simulation running..." indicator, fetches the external data once (`.jshd` as base64, `forevertree.jshc` as text → keyed by virtual filename `{"temperature.jshd", "precipitation.jshd", "forevertree.jshc"}`; the engine resolves `external temperature` to the virtual file `temperature.jshd`), calls `WasmLayer.runSimulation(code, "Main", externalData, …, false, "")` for one replicate, then hides the indicator and stores the `SimulationResult` via `getLastResult()` (errors shown inline; button always re-enabled). The two climate `.jshd` were generated via `josh preprocess` (per `test.sh`) and committed to `paper/forevertree/data/` (root `*.jshd` ignore negated for them); `install_deps.sh` copies them + the config into the gitignored `demo/data/`. Visualization is Component 7; CI WASM-embed is Component 10.
...
- **`viz.js` `GridPresenter` is forked, not verbatim.** The demo's copy was changed so the heatmap color domain is **fixed across all timesteps** (computed over every year, not just the current one) so the map is comparable while scrubbing — the editor rescales per timestep. This (and the trimmed `MapConfigPresenter`/scroll-listener from Component 7) means `demo.joshsim.org/js/viz.js` is no longer a byte-identical copy of `editor/js/viz.js`. Fold into the shared-`viz` discussion above (a fixed-domain option could be upstreamed to the editor's `GridPresenter` as a flag).
...
- **Follow-up — make ForeverTree the canonical "bring your own data" tutorial.** What we've built here (the ForeverTree example whose distinguishing feature is reading external gridded temperature/precipitation rasters) should really become the guides' **"bring your own data"** tutorial — the canonical example of pulling real geospatial data into a Josh model. Add it alongside `hello` / `grass_shrub_fire` / `two_trees`: a hand-authored `landing/guides/` page linked from `landing/guide.html`, a `docs/guide/md/…md`, and wiring its example/data into the guide build (mirror how the others copy `examples/guide/*.josh` and preprocess tutorial data in CI). Reuse/link the External-Data figures, the `.jshd` data, and the WASM playground built here. The tutorial can briefly note that these climate figures are the paper's Figure 1 panels. Not done yet — separate from this demo PR.
