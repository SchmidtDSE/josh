...
public abstract class Distribution extends EngineValue {
   ...
   * selection propotional to the frequency with which that value appears in the distribution.
   *
   ...
   * sampling this draw can either happen with or without replacement. With replacement, each value
   * has a probability of selection proportional to the frequency of that value. Without
   * replacement, each value has a probability of selection proprotion to the frequency of that
   * value minus the number of times it was drawn in this invocation of getContents.
   *
   ...
   * @throws IllegalArgumentException if count exceeds the cardinality of this distribution and
   *     sampling without replacement (like there are too few elements if the distribution is
   *     realized).
   */
   ...
   *
   * @returns sum value of all elements in this distribution either actual or hypothetical depending
   *     on distribution type. Empty if not defined for this distribution.
   */
