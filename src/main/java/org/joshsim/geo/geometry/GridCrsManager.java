...
public class GridCrsManager {
  ...
  */
  ...
  //   affineParams.parameter("elt_1_1").setValue(-cellSize);       // Y scale (negative to flip Y)
  ...
  public MathTransform createGridToTargetCrsTransform(CoordinateReferenceSystem targetCrs)
    ...
    if (targetCrs.equals(baseCrs)) {
      ...
      // DEBUG
      // TODO - CANNOT convert from base CRS to target CRS, with VANILLA (non-grid) CRS 
      // Test is from WGS84 to UTM11N, in valid range
      
      // Test known point - IMPORTANT: Apache SIS expects (longitude, latitude) order
      double[] srcPt = new double[] {-115.0, 35.0};
      double[] dstPt = new double[2];
      
      try {
        baseCrsToTargetTransform.transform(srcPt, 0, dstPt, 0, 1);
